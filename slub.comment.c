 ...
 *   slab's freelist.
 *	一个slab被冻结后即从链表管理中移除.其仅存在于per cpu partial list中,仅有冻结其的
 * 处理器能对它进行链表操作.其他处理器能释放对象到其的freelist中但只有冻结其的处理器能
 * 从中取出对象.
 * 
 *   list_lock
 *
 ...
 * 	list_lock保护每个节点上的partial和full链表,以及partial slab计数器.锁定后链表中
 * 不能加入或移除某个slab,partial slab计数器也不能改变.要注意的是slab的total number作为一个原子值可以在不锁定的情况下更改?
 *
 ...
 * 	冻结的slab从链表中移除.slab被冻结意味着该slab被决定了某种用途,比如满足某个特定处理
 * 	器的分配.对象可以被释放到该slab中但跳过通常的链表操作.当该slab不再被需要,由持有
 * 该slab的处理器将该slab重新融进slab lists中.
 *
 *
 ...
 * 用于跟踪NUMA nodes中哪些已经分配了kmem_cache_nodes,是一个位图.
 */
 ...
 * freelist的指针,由于CONFIG_SLAB_FREELIST_HARDENED保护,可能不能直接解引用.
 */
 ...
 * random number.
 * CONFIG_SLAB_FREELIST_HARDENED开启后,freelist的指针为
 * ((ptr)^(&ptr)^(kmem_cache->random)
 */
...
//@order的slab有多少个大小为@size的对象
static inline unsigned int order_objects(unsigned int order, unsigned int size)
{
	return ((unsigned int)PAGE_SIZE << order) / size;
}

//kmem_cache->oo: 低16位为@order的slab有多少个大小为@size的对象,高16位为@order
...
#ifdef CONFIG_SLUB_CPU_PARTIAL
//以@nr_objects设置@s->cpu_partial以及@s->cpu_partial_slabs,后者是(以对象的两倍)粗略计算出的.
...
static void put_cpu_partial(struct kmem_cache *s, struct slab *slab, int drain)
{
	...
	if (oldslab) {
		if (drain && oldslab->slabs >= s->cpu_partial_slabs) {
			 ...
			 */
			//如果kmem_cache中slabs已经超过cpu_partial_slabs,
